...
/**
 * Tests for Client connector http version config.
 */
public class HttpVersionTestCase {

    private static Logger logger = LoggerFactory.getLogger(HttpVersionTestCase.class);
    ...
    @Test
    public void version_1_0_TestCase() {
        ...
        HTTPCarbonMessage response = getResponse(msg, false, HTTP_1_0);
        String result = new BufferedReader(new InputStreamReader(new HttpMessageDataStreamer(response)
                .getInputStream()))
                .lines().collect(Collectors.joining("\n"));

        assertEquals(testValue, result);
        assertEquals(response.getHeader(Constants.HTTP_VERSION), HTTP_1_0);
    ...
    @Test
    public void version_1_0_ChunkEnabledTestCase() {
        String HTTP_1_0 = "HTTP/1.0";
        HTTPCarbonMessage msg = new HTTPCarbonMessage(new DefaultHttpRequest(HttpVersion.HTTP_1_1,
                HttpMethod.POST, ""));
        msg.setHeader(Constants.HTTP_CONTENT_LENGTH, String.valueOf(testValue.length()));
        HTTPCarbonMessage response = getResponse(msg, true, HTTP_1_0);
        String result = new BufferedReader(new InputStreamReader(new HttpMessageDataStreamer(response)
                .getInputStream()))
                .lines().collect(Collectors.joining("\n"));
    ...
    @Test
    public void version_1_1_TestCase() {
        HTTPCarbonMessage msg = new HTTPCarbonMessage(new DefaultHttpRequest(HttpVersion.HTTP_1_1,
                HttpMethod.POST, ""));
        HTTPCarbonMessage response = getResponse(msg, true, Constants.DEFAULT_VERSION_HTTP_1_1);
        String result = new BufferedReader(new InputStreamReader(new HttpMessageDataStreamer(response)
                .getInputStream()))
                .lines().collect(Collectors.joining("\n"));

        assertEquals(testValue, result);
        assertEquals(response.getHeader(Constants.HTTP_VERSION), Constants.DEFAULT_VERSION_HTTP_1_1);
    ...
    private HTTPCarbonMessage getResponse(HTTPCarbonMessage msg, Boolean isEnabled, String version) {
